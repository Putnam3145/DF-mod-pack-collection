...
local initFileName='init.lua'
local mod_dir=dfhack.getDFPath().."/hack/mods"
local dfhack_init=dfhack.getDFPath().."/dfhack.init"
...
function checkInstalled(dfMod) --try to figure out if installed
    if dfMod.checkInstalled then
        ...
        if dfMod.raws_list then
			if type(dfMod.raws_list)=='table' then
				...
				for k,v in pairs(dfhack.internal.getDir(dfMod.path..dfMod.raws_list)) do
					if fileExists(dfhack.getDFPath().."raw/objects/"..v) then
						return true,v
					end
				end
...
function manager:init(args)
    ...
    for k,v in ipairs(mlist) do
        if v~="." and v~=".." and v~="init.lua" then
            ...
            if f then
				...
				modData.guard_dfhack_init={'#'..modData.guard[1],'#'..modData.guard[2]}
                modData.path=self.mod_dir.."/"..v..'/'
				modData.scripts_folder_name=modData.scripts_folder_name or modData.name
            end
        end
    end
    ---show thingy
    ...
    self:addviews{
        ...
        widgets.Panel{subviews={
            ...
            widgets.Label{
                text={
                {text="Install",key="CUSTOM_I",key_sep="()",enabled=self:callback("curModInstallable"),
...
function manager:curModInstallable()
	return not (self.selected.installed or self.selected.category)
end
...
function manager:install(trgMod,force)
    ...
    if trgMod.raws_list then
		if type(trgMod.raws_list)=='table' then
			for k,v in pairs(trgMod.raws_list) do
				copyFile(trgMod.path..v,dfhack.getDFPath().."/raw/objects/"..v)
			end
		else
			for k,v in pairs(dfhack.internal.getDir(trgMod.path..trgMod.raws_list)) do
				if v~="." and v~=".." then
					copyFile(trgMod.path..trgMod.raws_list.."/"..v,dfhack.getDFPath().."/raw/objects/"..v)
				end
			end
		end
    end
	if trgMod.scripts_list then
		if type(trgMod.scripts_list)=='table' then
			for k,v in ipairs(trgMod.scripts_list) do
				copyFile(trgMod.path..v,dfhack.getDFPath().."/hack/scripts/"..trgMod.scripts_folder_name.."_"..v)
			end
		else
			for k,v in pairs(dfhack.internal.getDir(trgMod.path..trgMod.scripts_list)) do
				if v~="." and v~=".." then
					copyFile(trgMod.path..trgMod.scripts_list.."/"..v,dfhack.getDFPath().."/hack/scripts/"..trgMod.scripts_folder_name..'_'..v)
				end
			end
		end
	end
    ...
    if trgMod.patch_dfhack_init then
		patchInit(dfhack_init,trgMod.guard_dfhack_init,trgMod.patch_dfhack_init)
	end
	...
	if trgMod.patch_dfhack_init then
		require('gui.dialogs').showMessage('Mod install','You must restart Dwarf Fortress to complete the mod install.')
	end
    print("done")
end
function manager:uninstall(trgMod)
    ...
    if trgMod.scripts_list then
		if type(trgMod.scripts_list)=='table' then
			for k,v in pairs(trgMod.scripts_list) do
				os.remove(dfhack.getDFPath().."/hack/scripts/"..v)
			end
		else
			for k,v in pairs(dfhack.internal.getDir(trgMod.path..trgMod.scripts_list)) do
				if v:find('.txt') then
    ...
    if trgMod.patch_dfhack_init then
		unPatchFile(dfhack_init,trgMod.guard_dfhack_init)
	end
